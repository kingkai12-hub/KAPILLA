...
// Move a position along route points by distanceMeters, returns new [lat, lng] and index
function advanceAlongRoute(
  ...
  if (!routePoints || routePoints.length < 2 || distanceMeters <= 0) {
  ...
  while (remaining > 0 && idx < routePoints.length) {
    ...
    if (segLen <= remaining) {
      curLat = routePoints[idx][0];
      curLng = routePoints[idx][1];
      remaining -= segLen;
      idx++;
    } else {
...
export default function VehicleTrackingMap({ waybillNumber }: { waybillNumber: string }) {
  ...
  // Smooth movement: anchor = last known server position + time it was received
  // The animation loop extrapolates forward from anchor using speed
  ...
  // Handle new data from server — update anchor
  const handleTrackingData = (data: TrackingData) => {
    ...
    // Update anchor — animation loop will extrapolate from here
    anchorRef.current = {
      lat: data.currentLat,
      lng: data.currentLng,
  ...
  useEffect(() => {
    const connectionStateRef = { current: { sseActive: false, pollingActive: false, isCleaningUp: false } };
    ...
    const fetchTrackingData = async () => {
      ...
      try {
        ...
        if (!connectionStateRef.current.isCleaningUp) setErrorText('Connection issue. Retrying...');
    ...
    const startPolling = () => {
      if (connectionStateRef.current.pollingActive || connectionStateRef.current.isCleaningUp) return;
    ...
    const stopPolling = () => {
      if (pollInterval) { clearInterval(pollInterval); pollInterval = null; connectionStateRef.current.pollingActive = false; }
    };
    ...
    const cleanupSSE = () => {
      if (es) {
        ...
        es = null; connectionStateRef.current.sseActive = false;
      }
    };

    const startHeartbeatMonitor = () => {
      heartbeatTimer = setInterval(() => {
        if (Date.now() - lastMessageTime > 30000 && connectionStateRef.current.sseActive) {
          cleanupSSE(); startPolling();
        }
      }, 5000);
    };
    ...
    try {
      ...
      es.onopen = () => { connectionStateRef.current.sseActive = true; lastMessageTime = Date.now(); stopPolling(); startHeartbeatMonitor(); };
      es.onmessage = (evt) => {
        lastMessageTime = Date.now();
        try { handleTrackingData(JSON.parse(evt.data)); } catch {}
      };
      es.onerror = () => { if (!connectionStateRef.current.isCleaningUp) { cleanupSSE(); startPolling(); } };
      fallbackTimer = setTimeout(() => {
        if (!connectionStateRef.current.sseActive && !connectionStateRef.current.isCleaningUp) { cleanupSSE(); startPolling(); }
      }, 3000);
    } catch { startPolling(); }
  ...
  // Smooth animation loop — extrapolates position from anchor using speed
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (anchor && !anchor.arrived && anchor.speedKmh > 0) {
        ...
        // Distance vehicle should have traveled since last server update
        const distMeters = (anchor.speedKmh / 3.6) * elapsedSec;

        let lat: number;
        let lng: number;

        if (anchor.routePoints && anchor.routePoints.length > 1) {
          // Move along actual road geometry
          [lat, lng] = advanceAlongRoute(anchor.routePoints, anchor.lat, anchor.lng, distMeters);
        } else {
          // Fallback: move in heading direction
          const headingRad = (anchor.heading * Math.PI) / 180;
          const metersPerDeg = 111320;
          lat = anchor.lat + (distMeters * Math.cos(headingRad)) / metersPerDeg;
          lng = anchor.lng + (distMeters * Math.sin(headingRad)) / (metersPerDeg * Math.cos((anchor.lat * Math.PI) / 180));
        }

        // Update every 4 frames (~15fps) — smooth but not wasteful
        frameCount++;
        if (frameCount % 4 === 0) {
        ...
        {!tracking?.routePoints && completedSegments.length > 0 && (
          <>
            {completedSegments.map((pos, idx) => (
              <DynamicRoutePolyline key={`comp-${idx}`} routePoints={pos as [number, number][]} currentPosition={displayPos} completedColor="#2563eb" remainingColor="#2563eb" completedWeight={8} remainingWeight={8} completedOpacity={0.9} remainingOpacity={0.9} showProgress={false} />
            ))}
            {remainingSegments.map((pos, idx) => (
              <DynamicRoutePolyline key={`rem-${idx}`} routePoints={pos as [number, number][]} currentPosition={displayPos} completedColor="#ef4444" remainingColor="#ef4444" completedWeight={6} remainingWeight={6} completedOpacity={0.5} remainingOpacity={0.5} showProgress={false} />
            ))}
          </>
        )}
        ...
        <div className="absolute inset-0 z-[2000] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl p-8 mx-4 max-w-sm w-full text-center">
            ...
            <p className="text-slate-600 text-sm mb-1">
              Shipment <span className="font-bold text-blue-600">#{waybillNumber}</span> has reached its destination.
            </p>
            <p className="text-slate-500 text-xs mb-6">A notification has been sent to the sender and receiver.</p>
            <div className="flex gap-3">
              <button onClick={() => setShowArrivalPopup(false)} className="flex-1 py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-2xl transition-colors text-sm">
                Confirm Arrival
              </button>
              <button onClick={() => setShowArrivalPopup(false)} className="py-3 px-4 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-2xl transition-colors text-sm">
                Dismiss
              </button>
            </div>
          </div>
