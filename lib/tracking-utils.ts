...
/**
 * Calculate heading (bearing) from point A to point B with robust edge case handling
 * @param fromLat Starting latitude
 * @param fromLng Starting longitude
 * @param toLat Ending latitude
 * @param toLng Ending longitude
 * @param fallbackHeading Optional fallback heading if calculation fails (default: 0)
 * @returns Heading in degrees (0-360), or fallbackHeading if points are too close
 */
export function calculateHeading(
  ...
  if (Math.abs(dy) < MIN_DISTANCE && Math.abs(dx) < MIN_DISTANCE) {
    // Points are too close to calculate meaningful heading
    return fallbackHeading;
  }
