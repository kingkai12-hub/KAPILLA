...
export function isInCity(
  lat: number,
  lng: number,
  zones: CityZone[] = TANZANIA_CITY_ZONES
): boolean {
  return zones.some(
...
export function applyJunctionSlowdown(
  ...
  if (
    isNearJunction(routePoints, currentIndex, currentLat, currentLng, config.junctionSlowdownRadius)
  ) {
...
export function handleTrafficStop(
  ...
  if (config.enableTrafficStops && Math.random() < config.stopProbability) {
    const stopDuration =
      config.stopDurationMin + Math.random() * (config.stopDurationMax - config.stopDurationMin);
...
export function calculateMovement(
  ...
  // Calculate actual time elapsed (allow catch-up for offline periods)
  // Cap at 5 minutes to prevent huge jumps if system was down for days
  const deltaSeconds = Math.max(1, Math.min(300, (now - lastUpdate) / 1000));
  ...
  // Apply smooth acceleration/deceleration
  const newSpeed = applySmoothAcceleration(state.speed, targetSpeed, deltaSeconds, config);
 ...
 */
export function getSpeedZoneDescription(lat: number, lng: number, speed: number): string {
