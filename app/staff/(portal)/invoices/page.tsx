...
import { Plus, FileText, CheckCircle, Clock, XCircle, Eye, Truck } from 'lucide-react';
...
export default function InvoicesPage() {
  ...
  const [filter, setFilter] = useState<'PROFORMA' | 'FINAL'>('PROFORMA');
  ...
  const fetchInvoices = async () => {
    try {
      const url = `/api/invoices-new?type=${filter}`;
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage proforma and final invoices</p>
        </div>
        <Link
          href={`/staff/invoices/create?type=${filter}`}
        ...
        >
          ...
          Create {filter === 'PROFORMA' ? 'Proforma' : 'Invoice'}
        </Link>
      </div>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-slate-500 mb-6">
              {searchQuery
                ...
                : `Create your first ${filter.toLowerCase()} invoice to get started`}
            </p>
            {!searchQuery && (
              <Link
                href={`/staff/invoices/create?type=${filter}`}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() =>
                            window.open(
                              `/api/invoices/${invoice.id}/pdf?type=delivery-note`,
                              '_blank'
                            )
                          }
                          className="inline-flex items-center gap-1 px-3 py-1 text-sm font-medium text-green-600 hover:text-green-700 hover:bg-green-50 rounded-lg transition-colors"
