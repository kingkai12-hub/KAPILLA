...
import { Package, Search, Trash2, FileText, CheckCircle, ScanLine, Edit, X } from 'lucide-react';
...
export default function ShipmentsPage() {
  ...
  const [expandedWaybill, setExpandedWaybill] = useState<string | null>(null);
  ...
  const handleDelete = async (waybill: string) => {
    ...
    try {
      ...
      alert('Error deleting shipment. Please check your connection and try again.');
    }
  };
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[860px] text-sm text-left">
            <thead className="bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 uppercase font-medium">
              <tr>
                <th className="px-3 sm:px-6 py-4 sticky left-0 bg-slate-50 dark:bg-slate-800 z-10">
                  Waybill
                </th>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {loading ? (
                ...
                filteredShipments.map((shipment) => (
                  <React.Fragment key={shipment.waybillNumber}>
                    <tr
                      onClick={() =>
                        setExpandedWaybill(
                          expandedWaybill === shipment.waybillNumber ? null : shipment.waybillNumber
                        )
                      }
                    ...
                    >
                      ...
                      <td className="px-3 sm:px-6 py-4 text-right">
                        <div className="text-slate-400 text-sm">
                          {expandedWaybill === shipment.waybillNumber ? '▼' : '▶'}
                        </div>
                      </td>
                    </tr>
                      ...
                      <tr className="bg-slate-50 dark:bg-slate-800/30">
                        <td colSpan={6} className="px-3 sm:px-6 py-4">
                          <div className="flex flex-wrap gap-3 justify-center">
                            {/* Track Shipment - Hide when delivered */}
                            {shipment.currentStatus !== 'DELIVERED' && (
                              <Link
                            ...
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                window.open(
                                  `/api/shipments/${shipment.waybillNumber}/pdf`,
                                  '_blank'
                                );
                              }}
                            ...
                            {shipment.currentStatus === 'DELIVERED' && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  window.open(
                                    `/staff/shipments/${shipment.waybillNumber}/pod`,
                                    '_blank'
                                  );
                                }}
                            ...
                            {userRole === 'ADMIN' && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleDelete(shipment.waybillNumber);
                                }}
