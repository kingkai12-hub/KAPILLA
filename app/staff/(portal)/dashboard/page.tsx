...
export default async function StaffDashboard() {
  ...
  try {
    ...
    deliveredToday = await withTimeout(
      db.shipment.count({
        where: {
    ...
    recentShipments = await withTimeout(
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: Truck,
      color: 'text-amber-600 dark:text-amber-100',
    ...
    },
    {
      ...
      icon: CheckCheck,
      color: 'text-emerald-600 dark:text-emerald-100',
      bg: 'bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-900/40 dark:to-emerald-800/40',
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex justify-between items-end relative z-10">
        <div>
          <h1 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 dark:from-blue-400 dark:to-indigo-400 tracking-tight drop-shadow-sm">
            Dashboard
          </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {stats.map((stat) => (
          <div
            key={stat.name}
            className={`relative overflow-hidden rounded-2xl border ${stat.border} ${stat.bg} p-6 shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-1 group`}
          >
            ...
            <div className="flex items-center justify-between relative z-10">
              ...
              <div
                className={`p-3 rounded-xl bg-white/60 dark:bg-black/20 backdrop-blur-sm ${stat.color} shadow-sm group-hover:scale-110 transition-transform duration-300`}
              >
